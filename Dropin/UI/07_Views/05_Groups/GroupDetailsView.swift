...
struct GroupDetailsView: View {
    ...
    var body: some View {
        VStack(alignment: .center) {
            HStack {
                Spacer()
                GroupView(name: group.name, color: Color(rgba: group.colorHex), hasDestructiveBt: false)
                Spacer()
            }
            .padding(.top, 15)
            .padding(.bottom, 20)
    ...
    init(group: SDGroup) {
        ...
        self._groupColor = State(initialValue: Color(rgba: group.colorHex))
    }
}
